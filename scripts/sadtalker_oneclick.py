...
import os, sys, subprocess, shutil, urllib.request, urllib.parse, json, glob, time
...
# 从 HuggingFace 下载（更稳定）
HF = "https://huggingface.co/OpenTalker/SadTalker/resolve/main"
models = [
    "SadTalker_V0.0.1.pth",
    "mapping_00109-model_dict-general_wi.pth",
    "mapping_00229-model_dict-general_wi.pth",
]
for name in models:
    out = f"checkpoints/{name}"
    ...
    try:
        ...
        gdrive_ids = {
            "SadTalker_V0.0.1.pth": "1g4d-H1kpV6BmM3sA7qRp2L9mhgVqGjvG",
            "mapping_00109-model_dict-general_wi.pth": "1dL7J5mQx3q7Xj5k7G5q3",
            "mapping_00229-model_dict-general_wi.pth": "1dL7J5mQx3q7Xj5k7G5q3",
        }
        fid = gdrive_ids.get(name)
        if fid:
            run(f"gdown --id {fid} -O {out} 2>/dev/null")
            if os.path.exists(out) and os.path.getsize(out) > 1e6:
                print(f"  Done ({os.path.getsize(out)//1024//1024}MB)")
...
cmd = (
    f"python inference.py --driven_audio \"{AUDIO_FILE}\" "
    f"--source_image \"anchor.jpg\" --result_dir \"./output\" "
    f"--preprocess crop --still --enhancer gfpgan"
)
run(cmd)
