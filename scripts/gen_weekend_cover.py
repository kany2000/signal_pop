#!/usr/bin/env python3
"""Signal Pop 周末特别版封面 — 复用 run_daily_cover.py 风格"""
...
OUT_DIR = "E:/projects/signal_pop/output/weekly_20260724"
...
def get_anchor():
    """Generate unique anchor via Pollinations AI — no reused stock photos"""
    seed = int(hashlib.md5((DATE + "_v3").encode()).hexdigest()[:8], 16) % 99999
    url = (f"https://image.pollinations.ai/prompt/"
           f"professional%20Chinese%20female%20news%20anchor%2C%20head%20and%20shoulders%20portrait%2C"
           f"%20natural%20realistic%20body%20proportions%2C%20neutral%20studio%20background%2C"
           f"%20professional%20broadcast%20look%2C%20soft%20lighting%2C%20no%20text%2C%20no%20distortion"
           f"?width=768&height=1080&nologo=true&seed={seed}")
    cache = os.path.join(CACHE, "anchor_ai.jpg")
    if not os.path.exists(cache):
        print(f"  Generating anchor via AI...")
...
def make_cover():
    print(f"Building weekend cover @ {W}x{H}...")
    ...
    draw.text((tx, 365), "周末特别版", fill=(100, 220, 255), font=sub_font)
    for ox, oy in [(1, 1), (-1, 1)]:
        draw.text((tx + ox, 365 + oy), "周末特别版", fill=(0, 100, 150, 60), font=sub_font)
...
def make_cover_portrait():
    ...
    draw.text((60, 50), "新闻 · 科技 · 财经", fill=(0, 200, 255), font=fnt(24))
    ...
    sub_font = fnt(34)
    draw.text((tx, 790), "周末特别版", fill=(100, 220, 255), font=sub_font)
    for ox, oy in [(1, 1), (-1, 1)]:
        draw.text((tx + ox, 790 + oy), "周末特别版", fill=(0, 100, 150, 60), font=sub_font)

    mark_font = fnt(36, bold=True)
    ...
    draw.text((PW - 60, PH - 45), "15条核心新闻", fill=(100, 120, 150), font=ft, anchor="rt")
