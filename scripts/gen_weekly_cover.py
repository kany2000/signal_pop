...
def make_cover():
    print(f"Building weekly cover @ {W}x{H}...")
    ...
    draw.text((tx, 345), "周末特别版 · 本周新闻汇总", fill=(100, 220, 255), font=sub_font)
    for ox, oy in [(1, 1), (-1, 1)]:
        draw.text((tx + ox, 345 + oy), "周末特别版 · 本周新闻汇总", fill=(0, 100, 150, 60), font=sub_font)
...
def make_cover_portrait():
    ...
    draw.text((60, 50), "新闻 · 科技 · 财经", fill=(0, 200, 255), font=ImageFont.truetype(FONT_BOLD, 24))

    tx = 80
    title_font = ImageFont.truetype(FONT_BOLD, 72)
    draw.text((tx, 680), "隔天信号弹", fill=WHITE, font=title_font)
    for ox, oy in [(2, 2), (-2, 2), (2, -2), (-2, -2)]:
        draw.text((tx + ox, 680 + oy), "隔天信号弹", fill=(0, 200, 255, 80), font=title_font)

    sub_font = ImageFont.truetype(FONT_REG, 34)
    draw.text((tx, 770), "周末特别版 · 本周新闻汇总", fill=(100, 220, 255), font=sub_font)
    for ox, oy in [(1, 1), (-1, 1)]:
        draw.text((tx + ox, 770 + oy), "周末特别版 · 本周新闻汇总", fill=(0, 100, 150, 60), font=sub_font)
