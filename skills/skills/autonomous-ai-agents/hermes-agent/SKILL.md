...
### Gateway Restart (Important: Use systemd, not process kill)

When restarting the gateway on a Linux system that runs it as a systemd user service, **always use systemd commands**. Direct process manipulation (`kill`, `nohup ... &`) and even `hermes gateway stop` can hang or leave the service in a failed state. The correct way:
...
# Start (after Hermes upgrade, this picks up the new version)
systemctl --user start hermes-gateway

# Restart (stop + start in one shot)
systemctl --user restart hermes-gateway

# If the service is stuck in failed state, reset first
systemctl --user reset-failed hermes-gateway
systemctl --user start hermes-gateway
```

After an upgrade, `hermes gateway start` internally calls `systemctl --user daemon-reload` so a plain start is sufficient — it will pick up the newly installed binary. The key diagnostic command when gateway won't start is `journalctl --user -u hermes-gateway -n 50` which shows the actual error messages from the service.
