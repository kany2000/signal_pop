#!/usr/bin/env bash
# =============================================================
# 每日版 · 应急分段渲染脚本（signal_pop）
# -------------------------------------------------------------
# 用途：标准管线 `tools/remotion_daily_build.py --render`（单次 remotion render）
#       在本环境会被后台任务回收（长渲染静默被杀）时使用。
#       本脚本把整片切成若干短段分别渲染，最后拼接 + 整体重编码，
#       规避单次长渲染被回收的问题。与 render_weekly_segmented.sh 同源。
#
# 用法：
#   bash tools/render_daily_segmented.sh <制作日> [CHUNK]
#     <制作日>  例如 20260901
...
#   * 总帧数自动从 remotion_poc/src/daily_segs.json 计算
...
DATE="${1:?用法: bash tools/render_daily_segmented.sh <制作日> [CHUNK]}"
CHUNK="${2:-1000}"
CRF="${CRF:-26}"

ROOT="E:/projects/signal_pop"
POC="$ROOT/remotion_poc"
OUT="$ROOT/output/daily/$DATE"
...
# 0) daily_segs.json 不存在则先导出（export_daily_remotion.py）
if [ ! -f "$POC/src/daily_segs.json" ]; then
  echo "[$(date +%H:%M:%S)] daily_segs.json 缺失，先跑 export_daily_remotion.py" | tee -a "$LOG"
  "$PY" "$ROOT/tools/export_daily_remotion.py" "$DATE" >> "$LOG" 2>&1 || { echo "!! 导出失败" >&2; exit 1; }
fi
...
if [ -n "${TOTAL_FRAMES:-}" ]; then
  ...
  TOTAL=$("$PY" -c "import json;d=json.load(open(r'$POC/src/daily_segs.json'));print(int(round(sum(x['dur'] for x in d)*30)))")
fi
...
for ((i=0;i<NPARTS;i++)); do
  ...
  ( cd "$POC" && "$REMIX" render DailyNews "$part" --frames="$start-$end" --concurrency=8 --browser-executable="$CHROME" ) >> "$LOG" 2>&1
...
if [ -f "$OUT/signal_pop_daily_$DATE.mp4" ] && grep -q "FINAL DONE" "$LOG" 2>/dev/null; then
  if check_part_frames "$OUT/signal_pop_daily_$DATE.mp4" "$TOTAL"; then
...
SILENT="$POC/out/DailyNews_silent.mp4"
FINAL="$OUT/signal_pop_daily_$DATE.mp4"
...
if [ ! -f "$AUDIO" ]; then
  echo "!! 缺少 TTS 音频 $AUDIO（请先跑 gen_cloud_tts.py）" >&2
  exit 1
fi
