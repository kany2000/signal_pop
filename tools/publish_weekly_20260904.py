#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Signal Pop 周末版定时发布 20260904 → 播报日 2026-09-05 08:00 (抖音+快手+B站, sau CLI)"""
...
# 快手：--headed
ks = read_md("kuaishou.md")
results["kuaishou"] = run("kuaishou", SAU + ["kuaishou", "upload-video", "--account", ACCOUNT,
    "--file", VIDEO, "--title", extract(ks, "标题"), "--desc", extract(ks, "简介"),
    "--tags", extract_tags(ks), "--thumbnail", COVER_34, "--schedule", SCHEDULE, "--headed"])
# B站：标题≤78字符；tag 固定；无封面
...
print(f"\n定时发布时间: {SCHEDULE}")
print("📌 知乎/Facebook/Twitter：sau 不支持，需手动。小红书/YouTube 可由 sau 发，按约定暂手动。")
