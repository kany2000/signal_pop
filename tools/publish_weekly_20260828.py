#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Signal Pop 周末特别版定时发布 20260828 → 8/29 周六 08:00 (抖音 + 快手 + B站，sau CLI)
注意：sau 当前支持 douyin/kuaishou/xiaohongshu/bilibili/tencent/youtube；
知乎/Facebook/Twitter 仍需手动发布。发布范围（按项目约定）：抖音/快手/B站 自动。
"""
...
DATE = "20260828"
OUT_DIR = "E:/projects/signal_pop/output/weekly/20260828"   # 文案与封面目录
VIDEO_DIR = "E:/projects/signal_pop/output/weekly/20260828"  # 视频目录
SAU = ["sau"]
ACCOUNT = "her2home"
SCHEDULE = "2026-08-29 14:00"
BILI_TID = 249

VIDEO = os.path.join(VIDEO_DIR, f"signal_pop_weekly_{DATE}.mp4")
COVER_34 = os.path.join(OUT_DIR, f"cover_weekly_{DATE}_3x4.png")
COVER_169 = os.path.join(OUT_DIR, f"cover_weekly_{DATE}_16x9.png")
...
# 1. 抖音 (headless + thumbnail 3:4；若封面弹窗不稳定则重试不带 --thumbnail)
...
print(f"\n定时发布时间: {SCHEDULE}")
print("📌 知乎/Facebook/Twitter：sau 不支持，需手动发布。小红书/YouTube 可由 sau 发，按项目约定暂手动。")
